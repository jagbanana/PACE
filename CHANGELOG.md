...
### Added
- **Status line under the agent's sign-off.** `pace_status` now returns
  a ready-to-echo `signature_status` string, e.g.
  `PACE Status: 🟢 | Beast Mode: ⚪`, which the generated CLAUDE.md tells
  the agent to append beneath its sign-off on every reply (using the
  value from the single session-start `pace_status` call — no
  recomputation, no extra calls). PACE Status is a traffic-light over
  the same vault health checks that fill `warnings`: 🟢 healthy · 🟡
  warnings · 🔴 errors. Beast Mode is 🟢 on · ⚪ off. `signature_status`
  is null when uninitialized (mid-onboarding), and the agent omits the
  line then. Bumps the CLAUDE.md template to v3; run `pace upgrade` in
  existing vaults to pick it up.
