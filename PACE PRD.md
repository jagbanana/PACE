...
### 5.2 Integration Layer — MCP Server + Skill (Cowork) / CLAUDE.md (Claude Code)

PACE is invoked by the model, not the user. The integration has two layers, and the second layer differs by client because Cowork and Claude Code load model-side instructions differently.

**1. The PACE MCP server** (`pace.mcp_server`) exposes a small set of tools. The client sees them as native tools the model can call any time. Tool surface in §6.8.

**2a. For Claude Cowork: the `pace-memory` Cowork plugin** bundles the MCP server config (`.mcp.json` at the plugin root) plus a model-facing skill (`skills/pace-memory/SKILL.md`) that tells the model when and how to invoke the tools. Cowork does **not** read project-scoped `.mcp.json` files — plugin packaging is the only Cowork-supported MCP integration path. The plugin ships with `system-prompts/{compact,review}.md` that scheduled tasks reference via `${CLAUDE_PLUGIN_ROOT}`.

**2b. For Claude Code: the in-vault `CLAUDE.md`** (auto-generated by `pace_init`) plus a vault-root `.mcp.json` give the same result through Claude Code's project-scoped MCP loading. Both the SKILL.md and CLAUDE.md tell the model:
...
Both paths are intentionally short — long instructions get diluted. The MCP tool descriptions carry most of the weight.

**Vault location resolution.** When the MCP server is launched once globally (the plugin case), it has no implicit cwd to use as a vault hint. Resolution chain: `PACE_ROOT` env var → `CLAUDE_PLUGIN_OPTION_VAULT_ROOT` (set by Cowork's `userConfig` if filled) → per-user config file (`%APPDATA%\pace\config.json` on Windows, `~/.config/pace/config.json` elsewhere, written by `pace_init`) → walk up from cwd looking for `system/pace_index.db` (the Claude-Code path) → uninitialized.
...
**Trigger:** the client opens in a folder where `pace_status` returns `initialized: false`. For Cowork users that's resolved via the per-user config file (§5.2); for Claude Code users it's the absence of `system/pace_index.db` walking up from cwd.

**Detection:** the SKILL.md (Cowork) or in-vault CLAUDE.md (Claude Code) instructs the model to call `pace_status` early. If the tool returns `{ "initialized": false }`, the model enters onboarding mode.

**Flow:**
1. Model introduces PACE in plain language, asks the user's name, an optional assistant nickname, and (for Cowork users without a pre-filled `vaultRoot`) where on disk to put the vault.
2. Model calls `pace_init(root="<path>")` to scaffold the folder structure, initialize the SQLite DB, generate `.gitignore`, write the in-vault `CLAUDE.md`, and record the vault path in the per-user config file.
3. Model calls `pace_capture` to save the user's name as the first long-term memory (and optionally the assistant nickname).
4. Model uses Cowork's `mcp__scheduled-tasks` MCP to register two scheduled tasks (daily compaction, weekly review), referencing the prompts at `${CLAUDE_PLUGIN_ROOT}/system-prompts/{compact,review}.md`. User sees these get created and can manage them in Cowork's task UI.
